...
//Адлегласць паміж дзвума кропкамі (x1,0) і (x2,y2)
function Dist(x1,x2,y2:real):real;
begin
  result:=sqrt(sqr(x2-x1)+y2*y2);
end;

//Пошук самая вялікая адлегласці для кропкі
function MaxForPoint(P:mas;x:real):real;
var
  i:integer;
  temp:real;
begin
  //Шукаем самую вялікую адлегласць
  result:=Dist(x,P[1,1],P[2,1]);
  for i := 2 to PCount do
  begin
    temp:=Dist(x,P[1,i],P[2,i]);
    if temp>result then          //Знайшлі вялікі - запісалі ў max
      result:=temp;
  end;//for
end;
